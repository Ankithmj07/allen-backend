...
export const getStudents = async (req: Request, res: Response) => {
  ...
  export const updateStudentProfile = async (req: Request, res: Response,next: NextFunction): Promise<void> => {
    try {
      const { email, gender, dob, pinCode, state, city, address1, address2, parentName, parentGender,  parentPhNumber } = req.body;
      ...
      const updateFields: Partial<{ gender: string; dob: Date; pinCode:number; state: string; city: string; address1: string; address2: string;
        parentName: string; parentGender:string; parentPhNumber:string;
       }> = {};
      ...
      if (parentName !== undefined) updateFields.parentName = parentName;
      if (parentGender !== undefined) updateFields.parentGender = parentGender;
      if (parentPhNumber !== undefined) updateFields.parentPhNumber = parentPhNumber;
